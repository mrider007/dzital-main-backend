...
class attributeController {
    ...
    async attributeAdd(req, res) {
        try {
            const categoryId = new mongoose.Types.ObjectId(req.body.category_id);
            const subcategoryId = new mongoose.Types.ObjectId(req.body.sub_category_id);
            let checkAttribute = await Attribute.find({ attribute: req.body.attribute, category_id: categoryId, sub_category_id: subcategoryId });
            if (!_.isEmpty(checkAttribute)) {
                res.status(400).send({ status: 400, message: 'Attribute Already Exists For This Category' });
            }
            else {
