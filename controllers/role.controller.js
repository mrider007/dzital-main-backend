    ...
    async roleAdd(req, res) {
        try {
            ...
            if (!_.isEmpty(roleCheck) && roleCheck._id) {
                res.status(400).send({ status: 400, message: 'Role Already Exists' });
            } else {
                ...
                if (!_.isEmpty(roleSave) && roleSave._id) {
                    ...
                    res.status(400).send({ status: 400, message: 'Role could not be Added' });
                }
            }
        } catch (e) {
        ...
        try {
            ...
            if (!_.isEmpty(roleDetails) && roleDetails._id) {
                ...
                if (!_.isEmpty(roleDelete) && roleDelete._id) {
                    res.status(200).send({ status: 200, data: roleDelete, message: 'Role Deleted Successfully' });
                } else {
