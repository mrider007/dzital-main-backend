...
class roleController {
    ...
    async roleAdd(req, res) {
        try {
            ...
            if (!_.isEmpty(roleCheck) && roleCheck._id) {
                res.send({ status: 201, data: {}, message: 'Role already exists' });
            } else {
                let roleSave = await Role.create(req.body);
                if (!_.isEmpty(roleSave) && roleSave._id) {
                    res.send({ status: 200, data: roleSave, message: 'Role saved successfully' });
                }
                else {
                    res.send({ status: 201, data: {}, message: 'Role could not be added' });
                }
            }
        } catch (e) {
    ...
    async roleList(req, res) {
        try {
            ...
            if (!_.isEmpty(roles)) {
                res.send({ status: 200, data: roles.docs, total: roles.total, limit: roles.limit, page: roles.page, pages: roles.pages, message: 'Role list fetched successfully' });
            } else {
                res.send({ status: 201, data: [], message: 'No role found' });
            }
        } catch (e) {
            res.send({ status: 500, message: e.message });
        }
    ...
    async roleDetails(req, res) {
        try {
            ...
            if (!_.isEmpty(roleInfo) && roleInfo._id) {
                res.send({ status: 200, data: roleInfo, message: 'Role details fetched successfully' });
            } else {
                res.send({ status: 201, data: {}, message: 'Role not found' });
            }
        } catch (e) {
            res.send({ status: 500, message: e.message });
        }
        ...
        try {
            ...
            if (!_.isEmpty(roleInfo) && roleInfo._id) {
                ...
                if (!_.isEmpty(roleUpdate) && roleUpdate._id) {
                    res.send({ status: 200, data: roleUpdate, message: 'Role has been updated' });
                } else {
                    res.send({ status: 201, data: {}, message: 'Role could not be updated' });
                }
            } else {
                res.send({ status: 201, message: 'Role not found' });
            }
        } catch (e) {
            res.send({ status: 500, message: e.message });
        }
        ...
        try {
            ...
            if (!_.isEmpty(roleDetails) && roleDetails._id) {
                ...
                if (!_.isEmpty(roleDelete) && roleDelete._id) {
                    res.send({ status: 200, data: roleDelete, message: 'Role has been deleted successfully' });
                } else {
                    res.send({ status: 201, message: 'Role could not be updated' });
                }
            } else {
                res.send({ status: 201, message: 'Role not found' });
            }
